...
def write_wav(audio):
    ...
    aid = audio["aid"]
    audio_done = False
    if os.path.exists(f"/home/keshawnhsieh/data/wenetspeech_l_wav/.{aid}.done"):
        audio_done = True
    ...
    for segment in segments:
        ...
        text = segment["text"]
        if len(set(["L", "DEV", "TEST_NET"]) & set(part)) >0 and len(re.sub("[\u4e00-\u9fa5]+", "", text)) == 0:
    ...
    if last_end != -1:
        ...
        if not audio_done:
            ...
            waveforms, sample_rate = torch.randn((1, last_end- very_start)), 48000
        for segment in filtered_segments:
            ...
            if "DEV" in part:
                ...
                continue
            if not os.path.exists(f"/home/keshawnhsieh/data/wenetspeech_l_wav/data_aishell/wav/{sub_dir}/{aid}"):
                os.system(f"mkdir -p /home/keshawnhsieh/data/wenetspeech_l_wav/data_aishell/wav/{sub_dir}/{aid}")
            save_path = f"/home/keshawnhsieh/data/wenetspeech_l_wav/data_aishell/wav/{sub_dir}/{aid}/{sid}.wav"
            if not audio_done:
                sox.save(save_path, audio, 16000, format="wav", bits_per_sample=16)
            text = sid + " " + text
            texts.append(text)
            # print(text)
    os.system(f"touch /home/keshawnhsieh/data/wenetspeech_l_wav/.{aid}.done")
    return texts
...
if __name__ == "__main__":
    ...
    # search all audios and count the number of L & dev and test_net
    count =0
    ...
    os.system(f"mkdir -p /home/keshawnhsieh/data/wenetspeech_l_wav/data_aishell/transcript")
    with open("/home/keshawnhsieh/data/wenetspeech_l_wav/data_aishell/transcript/aishell_transcript_v0.8.txt", "w") as f:
